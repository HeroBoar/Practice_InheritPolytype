...
public class PeopleTrack : People
{
    ...
    protected virtual void Track()
    {
        ...
        {
            if (people[i] == null || people[i].transform.name == "阿拉花官" || people[i].transform.name == "條子")
            {
                if (people[i] == null) distance[i] = 1000;      //如果人類死亡 距離 改為 1000
                else distance[i] = 999;                         //與阿拉花官物件的距離改為999
        ...
        int index = distance.ToList().IndexOf(min);         //索引值 = 轉清單.取得索引值(最小值)
